...
$max_cnum = getMaxCNum($curr_deck)['MAX(card_number)'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!empty($_POST['card-delete-btn'])) {
        deleteCard($curr_deck, $_POST['card-delete-id']);
        $cards_list = getDeckCards($curr_deck);
        $num_cards = getDeckSize($curr_deck)['size'];
        $max_cnum = getMaxCNum($curr_deck)['MAX(card_number)'];
    }
    if (!empty($_POST['update-deck-btn'])) {
        updateDeckInfo($curr_deck, $curr_user, $_POST['new-deck-title'], $_POST['new-deck-description']);
        $curr_deck_title = getDeckTitle($curr_deck);
        $curr_deck_description = getDeckDescription($curr_deck);
    }
    ...
    if (!empty($_POST['add-card-btn'])) {
        addCard($curr_deck, $_POST['new-card-term'], $_POST['new-card-def'], $max_cnum);
        $cards_list = getDeckCards($curr_deck);
        $num_cards = getDeckSize($curr_deck)['size'];
        $max_cnum = getMaxCNum($curr_deck)['MAX(card_number)'];
        //var_dump($num_cards);
