...
function addAccount($username, $pass, $pass_confirm) {
    ...
    if (!checkUsername($username)) {
        if ($pass === $pass_confirm) {
            ...
            $statement->bindValue(':user_password', $hash);
...
function login($username, $user_password) {
    ...
    if(!checkUsername($username)) {
        ...
        $statement->bindValue(':username', $username);
        $statement->execute();
        $hash = $statement->fetch()['user_password']; // hash from database
        $statement->closeCursor();
        $is_valid_pass = password_verify($user_password, $hash);
       // var_dump($hash);
        if ($is_valid_pass) {
            session_start();
...
function change_password($curr_user, $curr_pass, $new_pass, $new_pass_conf) {
    ...
    $statement->execute();
    $hash = $statement->fetch()['user_password'];
    $statement->closeCursor();
    // verify original hash
    $valid_old_pass = password_verify($curr_pass, $hash);
    if ($valid_old_pass) {
        if ($new_pass === $new_pass_conf) {
            $new_hash = password_hash($new_pass, PASSWORD_DEFAULT); // generate new password hash
            $query = "UPDATE user_main SET user_password=:user_password WHERE username=:username";
            $statement = $db->prepare($query);
            $statement->bindValue(':username', $curr_user);
            $statement->bindValue(':user_password', $new_hash);
            $statement->execute();
            $statement->closeCursor();
        }
        else {
...
function addNickname($curr_user, $nickname) {
    global $db;
    if (validateNicknames($curr_user, $nickname)) {
...
// make sure nickname doesn't already exist
function validateNicknames($curr_user, $nickname) {
    global $db;
    $query = "SELECT COUNT(*) FROM user_nicknames WHERE username=:username AND nickname=:nickname";
    $statement = $db->prepare($query);
    $statement->bindValue(":nickname", $nickname);
    $statement->bindValue(":username", $curr_user);
    $statement->execute();
...
function sendRequest($recipient, $message, $request_date, $username) {
    ...
    if (checkUsername($recipient)) {
        if (validateRecipient($recipient, $username)) {
            if (validateRequest($recipient, $username)) {
                ...
                echo "
                    <script>
                        alert(\"Request Invalid! (Either you have already sent this user a request or this user has already sent a request to you.)\");
                    </script>
                ";
            }
        }
            ...
            echo "
                <script>
                    alert(\"Recipient is already your friend!\");
                </script>
            ";
        }
    }
...
function validateRecipient($recipient, $username) {
    global $db;
    $query = "SELECT COUNT(*) FROM user_friend_names WHERE username=:username AND friend_name=:friend_name";
    $statement = $db->prepare($query);
...
function validateRequest($recipient, $username) {
    global $db;
    $query = 'SELECT COUNT(*) FROM request WHERE username=:username AND recipient=:recipient';
    $statement = $db->prepare($query);
    $statement->bindValue(':recipient', $recipient);
    $statement->bindValue(':username', $username);
    $statement->execute();
    ...
    $statement->bindValue(':recipient', $username);
    $statement->execute();
...
function createDeck($title, $description, $creation_date, $username) {
    global $db;
    // First check to see if deck title already exists for the given user
    $query = "SELECT COUNT(deck_id) FROM deck WHERE title=:title AND username=:username";
    $statement = $db->prepare($query);
...
function updateDeckInfo($deck_id, $username, $title, $description) {
    global $db;
    // First check to see if deck title already exists for the given user
    $query = "SELECT COUNT(deck_id) FROM deck WHERE title=:title AND username=:username AND deck_id!=:deck_id";
    $statement = $db->prepare($query);
    $statement->bindValue(":title", $title);
    $statement->bindValue(":username", $username);
    $statement->bindValue(":deck_id", $deck_id);
...
function updateCard($deck_id, $card_number, $term, $definition) {
    global $db;
    if (validateCard($deck_id, $definition)) {
        ...
        echo "
                <script>
                    alert(\"Warning! Duplicate card definitions not allowed!\");
                </script>
            ";
    }
}
...
function validateCard($deck_id, $definition) {
    // duplicate definitions not allowed
    global $db;
    $query = "SELECT COUNT(*) FROM card WHERE deck_id=:deck_id AND definition=:definition";
    $statement = $db->prepare($query);
    $statement->bindValue(":deck_id", $deck_id);
    $statement->bindValue(":definition", $definition);
    $statement->execute();
...
function createQuiz($q_title, $deck_title, $length, $quiz_date, $username) {
    ...
    $query = "SELECT COUNT(quiz_id) FROM quiz WHERE q_title=:q_title AND username=:username";
    $statement = $db->prepare($query);
