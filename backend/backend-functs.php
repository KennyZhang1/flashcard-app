...
function deleteCard($deck_id, $card_number, $curr_size) {
    global $db;
    ...
    $statement->execute();
    $statement->closeCursor();
    $query = "UPDATE deck SET size=:size WHERE deck_id=:deck_id";
    $statement = $db->prepare($query);
    $statement->bindValue(":size", $curr_size-1);
    $statement->bindValue(":deck_id", $deck_id);
    $statement->execute();
    $statement->closeCursor();
}
...
function addCard($deck_id, $term, $definition, $deck_size) {
    global $db;
    $card_number = $deck_size + 1;
    ...
    $statement = $db->prepare($query);
    $statement->bindValue(":deck_id", $deck_id);
    $statement->bindValue(":size", $card_number);
    $statement->execute();
    $statement->closeCursor();
}
